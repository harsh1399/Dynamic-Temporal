...
with open(f"new_link_timeline2.json") as f:
    links = json.load(f)

titles = links.keys()

templates = ['officeholders_remaining1']
global_keys = list()
    ...
    for page_title in titles:
        ...
        for rev_num, link in enumerate(links[page_title]):
            try:
                ...
                #url = f'https://en.wikipedia.org/w/index.php?title=United_States&action=raw'
                driver.get(url)
                ...
                for key in json_dict:
                    if key=='population_estimate' or key =='GDP_PPP' or key=='GDP_PPP_per_capita' or key=='GDP_nominal' or key =='GDP_nominal_per_capita':
