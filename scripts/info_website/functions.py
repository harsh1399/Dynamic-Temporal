...
def clean(text):
    ...
    result = replace_stars_new(result)
...
def modified_cleaning(text):
    text = re.sub(r'\{\{\s*cite.*\}\}','',text)
    text = re.sub(r'\{\{\s*url\s*=\s*.*\s*\}\}','',text)
    text = re.sub(r'\{','',text)
    text = re.sub(r'\}','',text)
    text = re.sub('increase','',text)
    text = re.sub('decrease','',text)
