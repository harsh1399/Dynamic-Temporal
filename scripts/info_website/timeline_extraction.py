...
templates = ['new_website']
global_keys = list()
for template in templates:
    try:
        os.mkdir(f"infoboxes/{template}")
    ...
    for page_title in titles:
        ...
        prev_json = dict()
        links[page_title] = links[page_title][-2000:]
        for rev_num, link in enumerate(links[page_title]):
            try:
                ...
                #url = f'https://en.wikipedia.org/w/index.php?title=Twitter&action=history&action=raw'
                driver.get(url)
                ...
                try:
                    ...
                    pass
                #infobox_keys = re.findall("\|\s*[a-zA-Z]+\s*_*[a-zA-Z]*\s*=",infobox)
                #info_keys = re.findall('\^(.*?)ZXC',replace_tabs(infobox+' | '))
                infobox_keys = re.findall("<>\|\s*[a-zA-Z0-9_]*\s*=", infobox)
                ...
                for key_num,key_value in enumerate(infobox_keys):
                    try:
                        ...
                        if (key_num + 1) < len(infobox_keys):
                            #print((infobox_keys[key_num+1]).split('=')[0].strip())
                            new_val = infobox.split((infobox_keys[key_num+1]).split('=')[0].strip())[0]
                ...
                for key in json_dict:
                    if key == 'revenue' or key == 'net_income' or key == 'operating_income' or key == 'assets' or key == 'equity':
                        json_dict[key] = modified_cleaning(json_dict[key])
                ...
                # expMod(json_dict)
                # statMod(json_dict)
                # aflstatMod(json_dict)
                # cflstatMod(json_dict)
                # except:
                #     print('fail')
