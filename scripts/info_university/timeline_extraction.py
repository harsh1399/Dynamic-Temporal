...
with open(f"new_link_timeline1.json") as f:
    links = json.load(f)

titles = links.keys()

templates = ['universities_remaining_1']
global_keys = list()
for template in templates:
    try:
        os.mkdir(f"infoboxes/{templates}")
    ...
    for page_title in titles:
        ...
        for rev_num, link in enumerate(links[page_title]):
            try:
                ...
                url = f'https://en.wikipedia.org/w/index.php?title=University_of_Michigan&action=history&action=raw'
                driver.get(url)
                ...
                for key_num,key_value in enumerate(infobox_keys):
                    try:
                        ...
                        #key_val = infobox.split('|')[key_num+1]
                        #val = infobox.split(key_value)[1]
                        #new_key = key_val.split('=')[0]
